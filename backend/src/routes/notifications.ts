...
// GET /api/notifications/vapid-key - Get the public VAPID key
router.get('/vapid-key', async (req: Request, res: Response): Promise<any> => {
  try {
...
// POST /api/notifications/register-push - Register a new subscription for Web Push
router.post('/register-push', async (req: Request, res: Response): Promise<any> => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
