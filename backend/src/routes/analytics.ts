...
// Get spending distribution by category (Pie Chart)
router.get('/distribution', async (req: Request, res: Response): Promise<any> => {
  try {
...
// Get AI financial insights with caching
router.get('/insights', async (req: Request, res: Response): Promise<any> => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
...
// Get daily, weekly, monthly, yearly, and total spending
...
// Get monthly trend (Bar Chart data, last 6 months)
router.get('/monthly-trend', async (req: Request, res: Response): Promise<any> => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
    const userId = new mongoose.Types.ObjectId(req.user.id);
...
// Get daily trend (Line Chart data, last 30 days)
router.get('/daily-trend', async (req: Request, res: Response): Promise<any> => {
  try {
    if (!req.user) return res.status(401).json({ error: 'Unauthorized' });
    const userId = new mongoose.Types.ObjectId(req.user.id);
...
// Get financial health score and metrics breakdown
